...
export default function AdminDashboard() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Show success message and keep button disabled until it disappears
      setSuccessMessage("Employee added successfully!");
      setTimeout(() => {
        setSuccessMessage("");
        setIsSubmitting(false); // Enable button only after message disappears
      }, 3000);
      ...
      setIsSubmitting(false); // Enable button on failure
    }
  };
  ...
  const activeEmployeesCount = employees.filter(emp => {
    ...
    if (empTasks.length === 0) return true; 
